...
export default function EditRowForm({ row, products, categories, onSave, onCancel }:
  {
    row: ShoppingListRow,
    products: Product[],
    categories: Category[],
  ...
  })
{
  ...
  const [category, setCategory] = useState(row.product.category.id);
    ...
    <form action={ formAction } onSubmit={ onSubmit }>
      <div className="rounded-md border bg-gray-50 p-4 md:p-6">
        ...
        <div className="mt-2 flex justify-end gap-4">
          ...
          <Button type="submit" aria-disabled={ isPending }>Save Row</Button>
        </div>
      </div>
    </form>
  )
